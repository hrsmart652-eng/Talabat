  ...
  try {
    ...
    if (!profile) {
      return res
        ...
        .json(Jsend.fail({ message: "Driver profile not found" }));
    }
...
const setAvailability = async function (req, res, next) {
  try {
    const profile = await DriverProfile.findOne({ user_id: req.user._id });
    ...
    profile.is_available = req.body.is_available;
...
const updateLocation = async function (req, res, next) {
  try {
    ...
    profile.current_lat = req.body.current_lat;
    profile.current_lng = req.body.current_lng;
